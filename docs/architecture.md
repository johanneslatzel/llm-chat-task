...
fields. See the full field table in the [API Reference](api-reference.md#taskpool).
...
defaults table in the [API Reference](api-reference.md#taskconfiguration).
...
- `createTask(input)`: validates and creates a `ready` task from a
  ...
  priority defaults to `low` (stored tasks keep their stored priority; no
  default is applied)
...
- `json-file-store`: the `ObjectStore` backend every pool is backed by
  (in-memory by default, or a `JsonFileStore` when `dir` is configured)
